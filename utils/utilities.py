...
def tf_visualizer_scalar(ax, scalar_tf):
    
    if isinstance(scalar_tf, torch.Tensor):
        scalar_tf = scalar_tf.detach().cpu().numpy()
    x = scalar_tf[:, 0]
    y = scalar_tf[:, -1]
    ...
    # Fill with colors
    for i in range(1, len(scalar_tf)):
        lower = scalar_tf[i-1][0]
        upper = scalar_tf[i][0]
        out = ax.fill_between(x, y,
                              where=((lower<=x) & (x<=upper)),
                              interpolate=False, step='mid', color=scalar_tf[i][1:-1])

    return out

def tf_visualizer_gradient(ax, grad_tf):
    if isinstance(grad_tf, torch.Tensor):
        grad_tf = grad_tf.detach().cpu().numpy()

    base_color = [0.9, 0.55, 0.45]
    x = grad_tf[:, 0]
    y = grad_tf[:, 1]
    ...
    for i in range(1, len(grad_tf)):
        lower = grad_tf[i-1][0]
        upper = grad_tf[i][0]
        out = ax.fill_between(x, y,
                              where=((lower<=x) & (x<=upper)),
                              interpolate=False, step='mid', color=base_color + [grad_tf[i][1]], edgecolor=None)

    # # Fill with color
    # out = ax.fill_between(x, y,
    #     where=(0.2>y),
    #     interpolate=False, step='mid', color=base_color + [0.])
    
    # out = ax.fill_between(x, y,
    #     where=((0.2<y) & (y<=0.4)),
    #     interpolate=False, step='mid', color=base_color + [0.4])
    
    # out = ax.fill_between(x, y,
    #     where=((0.4<y) & (y<=0.6)),
    #     interpolate=False, step='mid', color=base_color + [0.6])
    
    # out = ax.fill_between(x, y,
    #     where=((0.6<y) & (y<=0.8)),
    #     interpolate=False, step='mid', color=base_color + [0.8])
    
    # out = ax.fill_between(x, y,
    #     where= 0.8<y,
    #     interpolate=False, step='mid', color=base_color + [1.])
