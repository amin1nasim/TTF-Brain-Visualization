...
class TF_for_VR2(nn.Module):
    ...
    def forward(self, x_input, _=None):
        ...
        ['render_slice', 'adaptive_tf_sampler', 'tf_sampler', 'visualize_transfer_func'] or  self.__dim_in == x_input.shape[1]\
...
class VolumeRenderer4(nn.Module):
    ...
    def render_slice(self, dim, index):
        assert self.tf_mode in ['custom2', 'custom', 'truth'], 'Set transfer function mode (custom2 ,custom, truth).'
        assert isinstance(dim, (int, torch.Tensor)), f'Argument dim should be either int, or torch.Tensor. Received {type(dim)}.'
        assert isinstance(index, torch.Tensor), f'Argument index should be torch.Tensor. Received {type(index)}.'
        ...
        rv = self.volume[self.volume_idx, 0]
        slices = rv.index_select(dim, index)
        # Bring dim to first axis
        permute_indices = list(range(slices.dim()))
        permute_indices.insert(0, permute_indices.pop(dim))
        slices = slices.permute(permute_indices)

        b, h, w = slices.shape
        
        slices = T(slices.reshape(-1,1), None)[:, :-1].reshape(b, h, w, 3).permute(0, 3, 1, 2)
        return slices
